...
export default async function Index() {
  ...
  const { data: joinedRides } = await supabase
    .from("requests")
    .select(
      "ride(id, destination, source, date, time, transit, capacity, guests, owner, users(name, phone, image, house, year)), approved"
    )
    .eq("requester", userId)
    .eq("approved", true)
    .order("created_at", { ascending: false });
  ...
  const myRidesFiltered = myRides?.filter((ride) => {
    if (
      ...
      convertDate(ride.date!) !== currentDate
    ) {
      return false;
    }
    return true;
  ...
  const joinedRidesFiltered = joinedRides?.filter((ride) => {
    if (
      new Date(ride.ride.date!) < new Date() &&
      convertDate(ride.ride.date!) !== currentDate
    ) {
      return false;
    }
    return true;
